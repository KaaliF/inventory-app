...
export default function Orders() {
  ...
  const getBadge = (paymentType) => {
    if (paymentType === 'cash') return { label: t('orders.cashBadge'), cls: 'bg-green-100 text-green-700' };
    if (paymentType === 'bank') return { label: t('orders.bankBadge'), cls: 'bg-blue-100 text-blue-700' };
    return { label: t('orders.creditBadge'), cls: 'bg-red-100 text-red-700' };
  };
    ...
    <div className="space-y-6">
      ...
      {orders.length === 0 ? (
        ...
        <div className="space-y-4">
          {orders.map((order) => {
            const badge = getBadge(order.paymentType);
            return (
