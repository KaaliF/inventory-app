...
export default function Customers() {
  ...
  const totalDebit = ledgerTxns.filter(tx => tx.category === 'sale').reduce((sum, tx) => sum + tx.amount, 0);
  const totalCredit = ledgerTxns.filter(tx => tx.category === 'payment_received').reduce((sum, tx) => sum + tx.amount, 0);
              ...
              {!ledgerLoading && ledgerTxns.length > 0 && (
                <div className="grid grid-cols-3 gap-3">
                  <div className="bg-red-50 rounded-xl border border-red-200 p-3">
                    <p className="text-xs text-red-600">{t('customers.totalSales')}</p>
                  ...
                  <div className="bg-green-50 rounded-xl border border-green-200 p-3">
                    <p className="text-xs text-green-600">{t('customers.totalReceived')}</p>
                  ...
                  <div className={`rounded-xl border p-3 ${netBalance > 0 ? 'bg-orange-50 border-orange-200' : 'bg-emerald-50 border-emerald-200'}`}>
                    <p className={`text-xs ${netBalance > 0 ? 'text-orange-600' : 'text-emerald-600'}`}>{t('customers.balance')}</p>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {(() => {
                        ...
                        return ledgerTxns.slice().sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)).map((tx, i) => {
                          const isDebit = tx.category === 'sale';
